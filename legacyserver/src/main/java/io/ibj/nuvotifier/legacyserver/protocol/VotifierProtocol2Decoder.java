...
public class VotifierProtocol2Decoder extends MessageToMessageDecoder<String> {
    ...
    private final V2TokenProvider tokenProvider;

    public VotifierProtocol2Decoder(V2TokenProvider tokenProvider) {
    ...
    protected void decode(ChannelHandlerContext ctx, String s, List<Object> list) throws Exception {
        ...
        // Get the key for the given service.
        Key key = tokenProvider.getKeyForService(serviceName);
        ...
        String address = votePayload.get("address").getAsString();
        long timestamp = votePayload.get("timestamp").getAsLong();

        // Create the vote.
        Vote vote = new Vote(serviceName, username, address, Long.toString(timestamp));
        list.add(vote);
